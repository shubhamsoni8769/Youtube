      ...
      <div className="col-span-10 px-10 ">
        <div>
          <input
            ...
            onFocus={() =>{
              setShowSuggestions(searchQuery === "" ? false : true)
            }
            }
            onBlur={() => setShowSuggestions(false)}
            className="w-1/2 h-10 border border-gray-400 rounded-l-full p-3"
            type="text"
          ></input>
          <button className="py-2 px-3 border border-gray-400 rounded-r-full text-l">
            🔍
          </button>
        </div>
        {showSuggestions && (
          <div className="fixed bg-white py-2 px-5 w-[33rem] border rounded-lg shadow">
            <ul>
              {suggestions.map((s) => (
                <li key={s} className="hover:bg-gray-100 cursor-default">
                  {s}
                </li>
              ))}
            </ul>
